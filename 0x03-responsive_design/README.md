...
1. Using the previous file as the base, in your 02-styles.css file update the .container selector by changing width to max-width

If you resize your browser, you should see that the content is resizing.

2. Whatever the browser you use, it’s a good idea from now on, to toggle the device view.
...
In your _02-1-styles.css_ file:
...
4. We want to have a clickable icon that shows and hide our navigation. We don’t want to use JavaScript but find a pure HTML / CSS way. We learned that input type checkbox have a checked - unchecked state. So we are going to use this for our menu.
...
5. Let’s now, use a little bit of CSS magic to create an “hamburger” icon just with CSS.
...
7. We have multiple ways to make the typography responsive. The basic way would be to create multiple media queries and set a different font-size. But because we are using REM that are based on 62.5% (defined in the html selector). Changing that value would change proportionally all font sizes.
...
8. in 08-styles.css, at the end of the /* Card WORK
...
9. in 09-styles.css, in the /* Footer section
